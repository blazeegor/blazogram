...
class Bot:
    ...
    async def send_all(
        ...
        video: Union[str, InputFile] = None,
        parse_mode: str = None,
        reply_markup: Union[
    ...
    async def send_message(
        ...
        return await self.methods.SendMessage(
            chat_id=chat_id,
            text=self.locale.translate(chat_id, text)
            if self.locale.check_user(chat_id)
            else text,
    ...
    async def send_chat_action(
        self, chat_id: Union[int, str], action: str, message_thread_id: int = None
    ) -> bool:
        return await self.methods.SendChatAction(
    ...
    async def get_updates(
        self, allowed_updates: list = None, offset: int = None
    ) -> list[Update]:
    ...
    async def answer_callback_query(
        ...
        return await self.methods.AnswerCallbackQuery(
            callback_query_id, text, url, show_alert
        )
