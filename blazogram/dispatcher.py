...
async def feed_update(update: dict, handlers: list, bot: Bot, fsm_storage: BaseStorage):
...
async def polling(bot: Bot, allowed_updates, self):
    while True:
        ...
        if updates:
            await asyncio.wait(
                [asyncio.create_task(feed_update(update, self.handlers, bot, self.fsm_storage)) for update in updates])
...
class Dispatcher:
    ...
    async def start_polling(self, bot: Bot, allowed_updates: list = None):
        await asyncio.wait([asyncio.create_task(polling(bot, allowed_updates, self)), asyncio.create_task(self.scheduler.start())])